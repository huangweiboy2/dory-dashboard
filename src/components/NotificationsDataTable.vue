...
export default {
  ...
  render (createElement, context) {
    ...
    const headers = [
      ...
      { value: 'read', text: '', sortable: false },
      { value: 'param.runName', text: '运行名称', sortable: false },
      { value: 'title', text: '标题', sortable: false },
      { value: 'username', text: '用户名', sortable: false },
    ...
    set(data, ['scopedSlots', 'item.read'], context.scopedSlots['item.read'] || ((config) => {
      if(config.item.readTime){
        return <v-chip small color="primary">已读</v-chip>
      }else{
        return <v-chip small color="error">未读</v-chip>
      }
    }))
    set(data, ['scopedSlots', 'item.username'], context.scopedSlots['item.username'] || ((config) => {
    ...
    return <DataTable class="steps-data-table" {...data} item-key="notificationID">
      {context.children}
    </DataTable>
  }
}
