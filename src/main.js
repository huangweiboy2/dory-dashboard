...
if (process.env.VUE_APP_BASE_WS_API.indexOf('ws') === 0 || process.env.VUE_APP_BASE_WS_API.indexOf('wss') === 0) {
  Vue.prototype.GLOBAL = process.env.VUE_APP_BASE_WS_API
} else {
  var baseUrl = window.location.origin
  if (baseUrl.indexOf('https') === 0) {
    Vue.prototype.GLOBAL = baseUrl.replace('https', 'wss') + process.env.VUE_APP_BASE_WS_API
  } else if (baseUrl.indexOf('http') === 0) {
    Vue.prototype.GLOBAL = baseUrl.replace('http', 'ws') + process.env.VUE_APP_BASE_WS_API
  }
}
