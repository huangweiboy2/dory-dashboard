  ...
  <page-card name="步骤">
    <v-card-text>
      <v-tabs v-model="tabValue">
        <v-tab>步骤执行次数(次)</v-tab>
        <v-tab @change="change2()">步骤执行平均时长(秒)</v-tab>
        <v-tab @change="change3()">步骤执行次数周环比(次)</v-tab>
        <v-tab @change="change4()">步骤执行状态比率(比率)</v-tab>
      </v-tabs>
        ...
        <v-tab-item>
          <v-form>
            <v-container class="d-flex flex-wrap">
              ...
              ></v-select>
              <v-select
                :items="stepActionsList"
                label="步骤类型"
                class="mr-8"
                dense
                small-chips
                multiple
                v-model="chartParams.stepActions"
                @change="getEcharts()"
              ></v-select>
            </v-container>
          </v-form>
          ...
          <v-form>
            <v-container class="d-flex flex-wrap">
              ...
              <v-select
                ...
                small-chips
                multiple
                v-model="chartParams2.stepActions"
                @change="getEcharts2()"
              ></v-select>
            </v-container>
          </v-form>
          ...
          <v-form>
            <v-container class="d-flex flex-wrap">
              ...
              <v-select
                ...
                small-chips
                multiple
                v-model="chartParams3.stepActions"
                @change="getEcharts3()"
              ></v-select>
            </v-container>
          </v-form>
            ...
            <v-container class="d-flex flex-wrap">
              <v-select
                :items="[
                  ...
                  { text: 'moduleType（模块类型）', value: 'moduleType' },
                  { text: 'moduleName（模块名）', value: 'moduleName' },
                  { text: 'stepAction（步骤类型）', value: 'stepAction' },
                ]"
                label="维度"
                class="mr-8"
                dense
              ...
              <v-select
                ...
                small-chips
                multiple
                v-model="chartParams4.stepActions"
                @change="getEcharts4()"
              ></v-select>
            </v-container>
          </v-form>
...
export default {
  ...
  created () {
    const vm = this
    request.get('/metrics/steps/options').then(response => {
      ...
      vm.stepActionsList = response.data.stepActions
    })
  },
  mounted () {
    const vm = this
    ...
    request.post('/metrics/steps/count', vm.chartParams).then(response => {
      vm.config = {
        type: 'line',
        data: {
  ...
  methods: {
    ...
    getEcharts2 () {
      ...
      if(vm.chartParams2.timeRageDays == 0 && vm.dates2.length == 0){
        ...
        request.post('/metrics/steps/duration', vm.chartParams2).then(response => {
          vm.config2 = {
            type: 'line',
            data: {
              labels: response.data.labels,
    ...
    getEcharts3 () {
      ...
      if(vm.chartParams3.timeRageDays == 0 && vm.dates3.length == 0){
        ...
        request.post('/metrics/steps/countWeekly', vm.chartParams3).then(response => {
          vm.config3 = {
            type: 'line',
            data: {
              labels: response.data.labels,
    ...
    getEcharts4 () {
      ...
      if(vm.chartParams4.timeRageDays == 0 && vm.dates4.length == 0){
        ...
        vm.statusResultsList.map((item,index) => {
          ...
          request.post(`/metrics/steps/status/${item}`, vm.chartParams4).then(response => {
            vm.config4[index] = {
              type: 'line',
              data: {
                labels: response.data.labels,
