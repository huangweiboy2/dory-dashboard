  ...
  <page-card name="容器发布">
    <v-card-text>
      <v-tabs v-model="tabValue">
        <v-tab>容器发布次数(次)</v-tab>
        <v-tab @change="change2()">容器发布平均时长(秒)</v-tab>
        <v-tab @change="change3()">容器发布周环比(次)</v-tab>
        <v-tab @change="change4()">容器发布执行状态比率(比率)</v-tab>
      </v-tabs>
        ...
        <v-tab-item>
          <v-form>
            <v-container class="d-flex flex-wrap">
              ...
              ></v-select>
              <v-select
                :items="isMeshEnablesList"
                label="是否网格发布"
                class="mr-8"
                dense
                small-chips
                multiple
                v-model="chartParams.isMeshEnables"
                @change="getEcharts()"
              ></v-select>
            </v-container>
          </v-form>
          ...
          <v-form>
            <v-container class="d-flex flex-wrap">
              ...
              <v-select
                ...
                small-chips
                multiple
                v-model="chartParams2.isMeshEnables"
                @change="getEcharts2()"
              ></v-select>
            </v-container>
          </v-form>
          ...
          <v-form>
            <v-container class="d-flex flex-wrap">
              ...
              <v-select
                ...
                small-chips
                multiple
                v-model="chartParams3.isMeshEnables"
                @change="getEcharts3()"
              ></v-select>
            </v-container>
          </v-form>
            ...
            <v-container class="d-flex flex-wrap">
              <v-select
                :items="[
                  ...
                  { text: 'isMeshEnable（是否网格发布）', value: 'isMeshEnable' },
                ]"
                label="维度"
                class="mr-8"
                dense
              ...
              <v-select
                ...
                small-chips
                multiple
                v-model="chartParams4.isMeshEnables"
                @change="getEcharts4()"
              ></v-select>
            </v-container>
          </v-form>
...
export default {
  ...
  created () {
    const vm = this
    request.get('/metrics/deploys/options').then(response => {
      ...
      vm.isMeshEnablesList = response.data.isMeshEnables
    })
  },
  mounted () {
    const vm = this
    ...
    request.post('/metrics/deploys/count', vm.chartParams).then(response => {
      vm.config = {
        type: 'line',
        data: {
  ...
  methods: {
    ...
    getEcharts2 () {
      ...
      if(vm.chartParams2.timeRageDays == 0 && vm.dates2.length == 0){
        ...
        request.post('/metrics/deploys/duration', vm.chartParams2).then(response => {
          vm.config2 = {
            type: 'line',
            data: {
              labels: response.data.labels,
    ...
    getEcharts3 () {
      ...
      if(vm.chartParams3.timeRageDays == 0 && vm.dates3.length == 0){
        ...
        request.post('/metrics/deploys/countWeekly', vm.chartParams3).then(response => {
          vm.config3 = {
            type: 'line',
            data: {
              labels: response.data.labels,
    ...
    getEcharts4 () {
      ...
      if(vm.chartParams4.timeRageDays == 0 && vm.dates4.length == 0){
        ...
        vm.statusResultsList.map((item,index) => {
          ...
          request.post(`/metrics/deploys/status/${item}`, vm.chartParams4).then(response => {
            vm.config4[index] = {
              type: 'line',
              data: {
                labels: response.data.labels,
