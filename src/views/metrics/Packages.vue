  ...
  <page-card name="镜像打包">
    <v-card-text>
      <v-tabs v-model="tabValue">
        <v-tab>镜像打包次数(次)</v-tab>
        <v-tab @change="change2()">镜像打包平均时长(秒)</v-tab>
        <v-tab @change="change3()">镜像打包次数周环比(次)</v-tab>
        <v-tab @change="change4()">镜像打包状态比率(比率)</v-tab>
        <v-tab @change="change5()">镜像打包平均大小(M)</v-tab>
        <v-tab @change="change6()">镜像打包总尺寸(M)</v-tab>
      </v-tabs>
...
export default {
  ...
  created () {
    const vm = this
    request.get('/metrics/packages/options').then(response => {
  ...
  mounted () {
    ...
    request.post('/metrics/packages/count', vm.chartParams).then(response => {
      vm.config = {
        type: 'line',
        data: {
  ...
  methods: {
    ...
    getEcharts2 () {
      ...
      if(vm.chartParams2.timeRageDays == 0 && vm.dates2.length == 0){
        ...
        request.post('/metrics/packages/duration', vm.chartParams2).then(response => {
          vm.config2 = {
            type: 'line',
            data: {
              labels: response.data.labels,
    ...
    getEcharts3 () {
      ...
      if(vm.chartParams3.timeRageDays == 0 && vm.dates3.length == 0){
        ...
        request.post('/metrics/packages/countWeekly', vm.chartParams3).then(response => {
          vm.config3 = {
            type: 'line',
            data: {
              labels: response.data.labels,
    ...
    getEcharts4 () {
      ...
      if(vm.chartParams4.timeRageDays == 0 && vm.dates4.length == 0){
        ...
        vm.statusResultsList.map((item,index) => {
          ...
          request.post(`/metrics/packages/status/${item}`, vm.chartParams4).then(response => {
            vm.config4[index] = {
              type: 'line',
              data: {
                labels: response.data.labels,
    ...
    getEcharts5 () {
      ...
      if(vm.chartParams5.timeRageDays == 0 && vm.dates5.length == 0){
        ...
        request.post('/metrics/packages/size', vm.chartParams5).then(response => {
          vm.configF = {
            type: 'line',
            data: {
              labels: response.data.labels,
    ...
    getEcharts6 () {
      ...
      if(vm.chartParams6.timeRageDays == 0 && vm.dates6.length == 0){
        ...
        request.post('/metrics/packages/sizeTotal', vm.chartParams6).then(response => {
          vm.config6 = {
            type: 'line',
            data: {
              labels: response.data.labels,
