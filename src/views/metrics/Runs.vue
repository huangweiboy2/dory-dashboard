  ...
  <page-card name="流水线">
    <v-card-text>
      <v-tabs v-model="tabValue">
        <v-tab>流水线执行次数(次)</v-tab>
        <v-tab @change="change2()">流水线执行平均时长(秒)</v-tab>
        <v-tab @change="change3()">流水线执行次数周环比(次)</v-tab>
        <v-tab @change="change4()">流水线执行状态比率(比率)</v-tab>
        <v-tab @change="change5()">红灯修复平均时长(分)</v-tab>
      </v-tabs>
...
export default {
  ...
  mounted () {
    ...
    request.post('/metrics/runs/count', vm.chartParams).then(response => {
      vm.config = {
        type: 'line',
        data: {
  ...
  methods: {
    ...
    getEcharts2 () {
      ...
      if(vm.chartParams2.timeRageDays == 0 && vm.dates2.length == 0){
        ...
        request.post('/metrics/runs/duration', vm.chartParams2).then(response => {
          vm.config2 = {
            type: 'line',
            data: {
              labels: response.data.labels,
    ...
    getEcharts3 () {
      ...
      if(vm.chartParams3.timeRageDays == 0 && vm.dates3.length == 0){
        ...
        request.post('/metrics/runs/countWeekly', vm.chartParams3).then(response => {
          vm.config3 = {
            type: 'line',
            data: {
              labels: response.data.labels,
    ...
    getEcharts4 () {
      ...
      if(vm.chartParams4.timeRageDays == 0 && vm.dates4.length == 0){
        ...
        vm.statusResultsList.map((item,index) => {
          ...
          request.post(`/metrics/runs/status/${item}`, vm.chartParams4).then(response => {
            vm.config4[index] = {
              type: 'line',
              data: {
                labels: response.data.labels,
    ...
    getEcharts5 () {
      ...
      if(vm.chartParams5.timeRageDays == 0 && vm.dates5.length == 0){
        ...
        request.post('/metrics/runs/fix', vm.chartParams5).then(response => {
          vm.configF = {
            type: 'line',
            data: {
              labels: response.data.labels,
