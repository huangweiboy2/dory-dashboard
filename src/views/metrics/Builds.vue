  ...
  <page-card name="构建">
    <v-card-text>
      <v-tabs v-model="tabValue">
        <v-tab>构建次数(次)</v-tab>
        <v-tab @change="change2()">构建平均时长(秒)</v-tab>
        <v-tab @change="change3()">构建周环比(次)</v-tab>
        <v-tab @change="change4()">构建执行状态比率(比率)</v-tab>
      </v-tabs>
          ...
          <v-form>
            <v-container class="d-flex flex-wrap">
              ...
              <v-select
                ...
                small-chips
                multiple
                v-model="chartParams2.buildEnvs"
                @change="getEcharts2()"
              ></v-select>
            </v-container>
          </v-form>
          ...
          <v-form>
            <v-container class="d-flex flex-wrap">
              ...
              <v-select
                ...
                small-chips
                multiple
                v-model="chartParams3.buildEnvs"
                @change="getEcharts3()"
              ></v-select>
            </v-container>
          </v-form>
            ...
            <v-container class="d-flex flex-wrap">
              <v-select
                :items="[
                  ...
                  { text: 'buildEnv（构建环境）', value: 'buildEnv' },
                ]"
                label="维度"
                class="mr-8"
                dense
              ...
              <v-select
                ...
                small-chips
                multiple
                v-model="chartParams4.buildEnvs"
                @change="getEcharts4()"
              ></v-select>
            </v-container>
          </v-form>
...
export default {
  ...
  created () {
    const vm = this
    request.get('/metrics/builds/options').then(response => {
      ...
      vm.buildEnvsList = response.data.buildEnvs
    })
  },
  mounted () {
    const vm = this
    ...
    request.post('/metrics/builds/count', vm.chartParams).then(response => {
      vm.config = {
        type: 'line',
        data: {
  ...
  methods: {
    ...
    getEcharts2 () {
      ...
      if(vm.chartParams2.timeRageDays == 0 && vm.dates2.length == 0){
        ...
        request.post('/metrics/builds/duration', vm.chartParams2).then(response => {
          vm.config2 = {
            type: 'line',
            data: {
              labels: response.data.labels,
    ...
    getEcharts3 () {
      ...
      if(vm.chartParams3.timeRageDays == 0 && vm.dates3.length == 0){
        ...
        request.post('/metrics/builds/countWeekly', vm.chartParams3).then(response => {
          vm.config3 = {
            type: 'line',
            data: {
              labels: response.data.labels,
    ...
    getEcharts4 () {
      ...
      if(vm.chartParams4.timeRageDays == 0 && vm.dates4.length == 0){
        ...
        vm.statusResultsList.map((item,index) => {
          ...
          request.post(`/metrics/builds/status/${item}`, vm.chartParams4).then(response => {
            vm.config4[index] = {
              type: 'line',
              data: {
                labels: response.data.labels,
