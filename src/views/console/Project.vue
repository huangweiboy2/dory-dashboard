<template>
  <div class="project pa-4">
    ...
    <div class="dialog-box">
      ...
      <v-dialog
        v-model="allotHostDialog"
        max-width="600px"
        persistent
      >
        <v-card>
          <v-card-title>
            <span class="headline">分配新主机</span>
          </v-card-title>
          <v-card-text>
            <v-alert icon="mdi-alert-circle" prominent text type="info">
              <small>分配新主机到{{allotHostForm.envName}}环境，进行流水线发布的时候，可以把制品发布到该主机。</small>
            </v-alert>
                ...
                <v-row>
                  <v-col cols="12">
                    <v-text-field
                      label="主机名标识（hostName）*"
                      dense
                      v-model="allotHostForm.hostName"
                      :rules="[v => !!v || '主机名为必填项']"
                      hint="全局唯一的主机名标识，建议格式为${projectName}-xxx，添加后不可修改，格式例如：test-project1-host01。"
                      persistent-hint
                    >
                    </v-text-field>
                  </v-col>
                ...
                <v-row class="mt-2">
                  <v-col cols="12">
                    <v-text-field
                      label="主机地址（hostAddr）*"
                      dense
                      v-model="allotHostForm.hostAddr"
                      :rules="[v => !!v || '主机地址为必填项']"
                      hint="主机地址，格式例如：192.168.0.1或者主机名。"
                      persistent-hint
                    >
                ...
                <v-row>
                  <v-col cols="12">
                    <v-text-field
                      ...
                      dense
                      type="number"
                      v-model="allotHostForm.hostPort"
                      :rules="[v => !!v || '主机ssh端口为必填项']"
                      hint="主机的SSH远程连接端口。"
                      persistent-hint
                    >
                ...
                <v-row>
                  <v-col cols="12">
                    <v-text-field
                      label="主机SSH远程登录用户名（hostUser）*"
                      dense
                      v-model="allotHostForm.hostUser"
                      :rules="[v => !!v || '远程登录用户为必填项']"
                      hint="主机的SSH远程连接登录用户名。"
                      persistent-hint
                    >
                ...
                <v-row>
                  <v-col cols="12">
                    <v-text-field
                      ...
                      dense
                      type="password"
                      v-model="allotHostForm.hostPassword"
                ...
                <v-row>
                  <v-col cols="12">
                    <v-select
                      ...
                      :items="[ { text: '是', value: true }, { text: '否', value: false } ]"
                      dense
                      v-model="allotHostForm.hostBecome"
                      :rules="[v => typeof(v)== 'boolean' || '是否需要切换用户为必选项']"
                      hint="如果ssh登录后需要切换用户，请设置为需要切换用户"
                      persistent-hint
                    >
                ...
                <v-row>
                  <v-col cols="12">
                    <v-text-field
                      ...
                      dense
                      type="password"
                      v-model="allotHostForm.hostBecomePassword"
      ...
      <v-dialog
        v-model="hostJoinDialog"
        max-width="600px"
      >
        <v-card>
          <v-card-title>
            <span class="headline">将主机加入其他项目</span>
          </v-card-title>
          <v-card-text>
            <v-alert icon="mdi-alert-circle" prominent text type="info">
              <small>将主机加入其他项目环境后，该项目所在的环境也能发布应用到该主机。（注意，如果修改该主机信息，其他项目环境的主机信息也会发生修改）</small>
            </v-alert>
                ...
                <v-row>
                  <v-col cols="12">
                    <v-select
                      ...
                      dense
                      :rules="[v => !!v || '项目必填项']"
                      v-model="hostJoinForm.projectName"
                      @change="projectChange(hostJoinForm.projectName)"
                    ></v-select>
                  </v-col>
                </v-row>
                <v-row>
                  <v-col cols="12">
                    <v-select
                      ...
                      label="请选择环境*"
                      dense
                      :disabled="hostJoinForm.projectName ? false: true"
                      :rules="[v => !!v || '环境必填项']"
                      v-model="hostJoinForm.envName"
                    ></v-select>
                  </v-col>
                </v-row>
                <v-row class="mb-2">
      ...
      <v-dialog
        v-model="hostLeaveDialog"
        max-width="600px"
      >
        <v-card>
          <v-card-title>
            <span class="headline">将主机从项目环境移除</span>
          </v-card-title>
          <v-card-text>
            <v-alert icon="mdi-alert-circle" prominent text type="error">
              <small>如果主机{{hostLeaveForm.hostName}}只配置在一个项目环境之下，将会删除该主机信息。<v-chip small color="red">危险操作，请输入主机名标识。</v-chip></small>
            </v-alert>
                ...
                <v-row>
                  <v-col cols="12">
                    <v-text-field
                      ...
                      required
                      dense
                      v-model="leaveHostName"
                      :rules="[v => !!v || '主机名称为必填项']"
                      :hint="`确认要将主机${hostLeaveForm.hostName}从项目环境移除吗？`"
                      persistent-hint
                    ></v-text-field>
                  </v-col>
                </v-row>
...
export default {
  ...
  created () {
    ...
    request.get(`/console/project/${projectName}`).then(response => {
    ...
    request.get('/console/userNames').then(response => {
    ...
    request.get('/console/projectEnvs').then(response => {
    ...
    request.get(`/console/project/${projectName}/envComponentTemplates`).then(response => {
      vm.defaultComponent = response.data.componentTemplateYaml
    }).catch(error => {
      vm.errorTip(true,error.response.data.msg)
    })
    request.get(`/console/project/${projectName}/pipelineTriggerStepActions`).then(response => {
  ...
  methods: {
    showLog (response) {
      ...
      if(response.data.withAdminLog){
        vm.disableBtn = true
        vm.ws = new WebSocket(`${vm.GLOBAL}/ws/log/audit/console/${response.data.auditID}?x-user-token=${vm.userToken}`)
    ...
    refreshList () {
      const vm =this
      vm.cardLoading = true
      request.get(`/console/project/${vm.targetProjectName}`).then(response => {
        vm.project = response.data.project
        vm.cardLoading = false
      }).catch(error => {
        vm.cardLoading = false
    ...
    updateToken () {
      const vm = this
      request.post(`/console/project/${vm.targetProjectName}/refreshToken`).then(response => {
    ...
    memberAdd () {
      const vm = this
      request.post(`/console/project/${vm.targetProjectName}/memberAdd`,vm.memberAddForm).then(response => {
    ...
    updateMember () {
      const vm = this
      request.post(`/console/project/${vm.targetProjectName}/memberUpdate`, vm.memberAddForm).then(response => {
    ...
    deleteMember () {
      const vm = this
      request.post(`/console/project/${vm.targetProjectName}/memberDelete`,{username: vm.targetUsername}).then(response => {
    ...
    addPipeline () {
      ...
      if(vm.$refs.addPipelineRef.validate()){
        request.post(`/console/project/${vm.targetProjectName}/pipelineAdd`,vm.addPipelineForm).then(response => {
    ...
    addPipelineEnv () {
      ...
      if(vm.$refs.addPipelineEnvRef.validate()){
        request.post(`/console/project/${vm.targetProjectName}/pipelineEnvAdd`, vm.addPipelineEnvForm).then(response => {
    ...
    refreshToken () {
      const vm = this
      request.post(`/console/project/${vm.targetProjectName}/pipelineTokenRefresh`, { branchName: vm.targetBranchName }).then(response => {
    ...
    changeSwitch(branchName,webhookPushEvent) {
      const vm = this
      request.post(`/console/project/${vm.targetProjectName}/pipelineGitWebhook`,{branchName,webhookPushEvent}).then(response => {
        vm.successTip(true,response.msg)
        vm.showLog(response)
      }).catch(error => {
        vm.errorTip(true,error.response.data.msg)
    ...
    cornAdd () {
      ...
      if(vm.$refs.cornAddRef.validate()){
        request.post(`/console/project/${vm.targetProjectName}/pipelineCronAdd`, vm.cronAddForm).then(response => {
          vm.successTip(true,response.msg)
    ...
    cronDelete () {
      const vm = this
      request.post(`/console/project/${vm.targetProjectName}/pipelineCronDelete`,{
    ...
    openAllotHost (envName) {
      this.allotHostDialog = true
      this.allotHostForm.envName = envName
      this.variableList = [['','']]
    },
    allotHost () {
      const vm = this
      if(vm.$refs.addHostRef.validate()){
        vm.allotHostForm.hostPort = Number(vm.allotHostForm.hostPort)
        vm.variableList.forEach(function(i) {
          vm.allotHostForm.variables[i[0]] = i[1];
        })
        request.post(`/console/project/${vm.targetProjectName}/envHostAdd`, vm.allotHostForm).then(response => {
          vm.allotHostDialog = false
          vm.successTip(true, response.msg)
          vm.refreshList()
        }).catch(error => {
          vm.errorTip(true, error.response.data.msg)
    ...
    modifyHost (envName,hostName) {
      const vm = this
      vm.hostConfigDialog = true
    ...
    updateHost () {
      ...
      if(vm.$refs.updateHostRef.validate()){
        vm.updateHostForm.hostPort = Number(vm.updateHostForm.hostPort)
        vm.variableList.forEach(function(i) {
          vm.updateHostForm.variables[i[0]] = i[1];
        })
        request.post(`/console/project/${vm.targetProjectName}/envHostUpdate`, vm.updateHostForm).then(response => {
          vm.hostConfigDialog = false
          vm.successTip(true, response.msg)
          vm.refreshList()
        }).catch(error => {
          vm.errorTip(true,error.response.data.msg)
    ...
    openHostJoin (hostName, envName) {
      const vm = this
      vm.hostJoinDialog = true
      vm.hostJoinForm.hostName = hostName
      request.get(`/console/project/${vm.targetProjectName}`).then(response => {
        response.data.project.projectAvailableEnvs.map(item => {
          if(item.envName == envName){
            item.hosts.map(i => {
              if(i.hostName == hostName){
                vm.variableList = Object.entries(i.variables)
                vm.hostJoinForm.groups = i.groups
              }
            })
          }
        })
    ...
    hostJoin () {
      ...
      if(vm.$refs.hostJoinRef.validate()){
        vm.variableList.forEach(function(i) {
          vm.hostJoinForm.variables[i[0]] = i[1];
        })
        request.post(`/console/project/${vm.targetProjectName}/envHostJoin`, vm.hostJoinForm).then(response => {
          vm.hostJoinDialog = false
          vm.successTip(true, response.msg)
          vm.refreshList()
        }).catch(error => {
          vm.errorTip(true,error.response.data.msg)
    ...
    openHostLeave (envName,hostName) {
      this.hostLeaveDialog = true
      this.hostLeaveForm.envName = envName
      this.hostLeaveForm.hostName = hostName
    },
    hostLeave () {
      const vm = this
      if(vm.$refs.hostLeaveRef.validate() && vm.hostLeaveForm.hostName == vm.leaveHostName){
        request.post(`/console/project/${vm.targetProjectName}/envHostLeave`, vm.hostLeaveForm).then(response => {
          vm.hostLeaveDialog = false
          vm.successTip(true, response.msg)
          vm.refreshList()
        }).catch(error => {
          vm.errorTip(true,error.response.data.msg)
        ...
        vm.warnTip(true, '请输入正确的主机名称')
      }
    },
    deleteHost(envName,hostName) {
    ...
    envHostDelete () {
      const vm = this
      if(vm.$refs.deleteHostRef.validate() && vm.deleteHostForm.hostName == vm.deleteHostName){
        request.post(`/console/project/${vm.targetProjectName}/envHostDelete`, vm.deleteHostForm).then(response => {
    ...
    dbAdd () {
      ...
      if(vm.$refs.dbAddRef.validate()){
        request.post(`/console/project/${vm.targetProjectName}/envDbAdd`, vm.DbAddForm).then(response => {
    ...
    updateDb () {
      ...
      if(vm.$refs.updateDbRef.validate()){
        request.post(`/console/project/${vm.targetProjectName}/envDbUpdate`, vm.updateDbForm).then(response => {
    ...
    dbJoin () {
      ...
      if(vm.$refs.dbJoinRef.validate()){
        request.post(`/console/project/${vm.targetProjectName}/envDbJoin`, vm.dbJoinForm).then(response => {
    ...
    dbLeave () {
      ...
      if(vm.$refs.dbLeaveRef.validate() && vm.dbLeaveForm.dbName == vm.leaveDbName){
        request.post(`/console/project/${vm.targetProjectName}/envDbLeave`, vm.dbLeaveForm).then(response => {
    ...
    deleteDb () {
      ...
      if(vm.$refs.deleteDbNameRef.validate() && vm.deleteDbForm.dbName == vm.deleteDbName){
        request.post(`/console/project/${vm.targetProjectName}/envDbDelete`, vm.deleteDbForm).then(response => {
    ...
    deletePipelineEnv () {
      const vm = this
      request.post(`/console/project/${vm.targetProjectName}/pipelineEnvDelete`, vm.deletePipelineEnvForm).then(response => {
    ...
    deleteDebug () {
      ...
      if(vm.$refs.deleteDebugRef.validate && vm.deleteDebugForm.envName == vm.deleteDebugName){
        request.post(`/console/project/${vm.targetProjectName}/envComponentDebugDelete`, vm.deleteDebugForm).then(response => {
    ...
    getDefaultComponent () {
      ...
      request.get(`/console/project/${vm.targetProjectName}/envComponentTemplates`).then(response => {
    ...
    chooseParams(e) {
      ...
      if (e === "杂项") {
        ...
        if (
          ...
          console.log(vm.addComponentForm.deploySpecStatic.securityContext);
          vm.addComponentForm.deploySpecStatic.securityContext = {
            runAsUser: 1000,
            runAsGroup: 0,
          };
    ...
    addComponent () {
      ...
      if(vm.$refs.addComponentRef.validate()){
        ...
        request.post(`/console/project/${vm.targetProjectName}/envComponentUpdate`, {
          envName: vm.targetEnvName,
    ...
    deleteComponent () {
      ...
      if(vm.$refs.deleteComponentRef.validate() && vm.deleteComponentName == vm.deleteComponentForm.componentName){
        request.post(`/console/project/${vm.targetProjectName}/envComponentDelete`, vm.deleteComponentForm).then(response => {
    ...
    triggerAdd () {
      ...
      if(vm.$refs.triggerAddRef.validate()){
        request.post(`/console/project/${vm.targetProjectName}/pipelineTriggerOp`, vm.triggerAddForm).then(response => {
    ...
    triggerUpdate () {
      ...
      if(vm.$refs.triggerUpdateRef.validate()){
        request.post(`/console/project/${vm.targetProjectName}/pipelineTriggerOp`, vm.triggerUpdateForm).then(response => {
    ...
    triggerDelete () {
      const vm = this
      request.post(`/console/project/${vm.targetProjectName}/pipelineTriggerDelete`, vm.triggerDeleteForm).then(response => {
